...
以下の分析結果をもとに、新規作成するLPのデザインで達成すべきゴールを定義してください。

【重要な前提】
このLPは、対象サービス/商品のために新規作成するものです。
入力されたURLは既存のLPではなく、サービス/商品の情報を理解するために参照したウェブサイトです。
...
※上記の追加情報を解釈する際の注意点：
- ユーザーは様々な言葉遣いやフォーマットで要望を記述している可能性があります
- 「〜な感じ」「〜っぽい」などの曖昧な表現も、デザイン意図として解釈してください
- 否定形の要望（「〜は避けたい」「〜は使わないで」）は禁止事項として反映してください
- 具体的な色名、フォント名、スタイル名が含まれていれば優先的に採用してください
- 参考サイトやブランド名が含まれていれば、そのデザイントーンを参考にしてください
- 追加情報の内容はデザインゴール・印象キーワードに必ず反映してください
...
以下のデザインゴールを達成するための、新規LP用のデザインコンセプトを提案してください。
...
- ユーザーは様々な言葉遣いやフォーマットで要望を記述している可能性があります
- 「〜な感じ」「〜っぽい」などの曖昧な表現も、デザイン意図として解釈してください
- 否定形の要望（「〜は避けたい」「〜は使わないで」）は禁止事項（prohibitions）に反映してください
- 参考サイトやブランド名が含まれていれば、そのデザイントーンをコンセプトに反映してください
- 追加情報の内容はコンセプトステートメント・原則に必ず反映してください
...
以下のデザインコンセプトを具現化するための、新規LP用の具体的なデザインガイドラインを作成してください。
...
- ユーザーは様々な言葉遣いやフォーマットで要望を記述している可能性があります
- 「〜な感じ」「〜っぽい」などの曖昧な表現も、デザイン意図として解釈してください
- 否定形の要望（「〜は避けたい」「〜は使わないで」）は、該当する色・フォント・スタイルを避けてください
- 具体的な色名（「青」「赤」など）が含まれていれば、適切なHEX値に変換して採用してください
- 具体的なフォント名が含まれていれば優先的に採用してください
- 参考サイトやブランド名が含まれていれば、そのデザイントーンを参考にしてください
- 追加情報の内容は配色・フォント・ビジュアルスタイルに必ず反映してください
